...
import { batchClaim, claimTokens, getPendingReward } from "../../utils/helpers/AirDropHelper";
...
export default function AirDrop() {
	...
	const fetchNftsData = async (acc) => {
		try {
			const response = await fetch(`https://nestjs-crash-production.up.railway.app/nft/list/${acc}`);
	...
	return (
		<Layout>
			{isLoading ? (
				<div className="air">
					<motion.h2 initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }} exit={{ opacity: 0, x: -40 }}>
						Air Drop
					</motion.h2>
				...
				>
					...
					<p className="big">
						40% tokens will be claimed immediately and 60% will accumulate over time. 60% will be released daily over a 12-month vesting period.
					</p>

					<div className="air__btn">
						<button style={{ marginRight: "4px" }} className="button light" onClick={claimReward} disabled={pendingReward <= 0}>
							Claim now
						</button>

						<button className="button light" onClick={batchClaimBtn}>
							Batch claim
						</button>
					</div>
				</motion.div>
